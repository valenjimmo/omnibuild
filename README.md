...
A multi-tenant ADU contractor workspace and homeowner portal, built from the supplied MVP design. Next.js 16, React 19, TypeScript, Supabase Auth/PostgreSQL/Storage, and PostgreSQL RLS. The UI uses custom responsive CSS and Lucide icons; Tailwind/shadcn are not dependencies.
...
Open http://localhost:3000. Without Supabase environment variables, the app explicitly runs in **Demo mode**, with sample projects and browser-local persistence. The homeowner demo is at `/portal/westwood-adu`. Demo identities and localStorage are not an authentication boundary. Files in this mode are limited to 2 MB and stored in this browser. Clear `omnibuild-demo-v1` from localStorage to reset.
...
6. Restart the app. Sign up, confirm email, sign in, and create a company. Add a client, create a project, and invite the client. Owners can invite staff in Settings. Invitations expire after seven days and are accepted only by the matching authenticated email.
...
- Contractor signup/login/logout and company creation; company name and logo URL; organization switching for users with multiple memberships.
...
`npm test` executes the migrations and 47 security assertions in embedded PostgreSQL (PGlite), plus input-validation tests. Auth and Storage infrastructure tables are minimal test fixtures, not the hosted Supabase services. The same SQL suite uses pgTAP against real local Supabase:
